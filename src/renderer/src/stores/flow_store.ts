...
export const useFlowStore = defineStore("flow", () => {
  ...
  const fullscreenAds = computed(() =>
    adsStore.getActiveAds.filter(
      (ad) => ad.display === "full" || ad.display === "topfull",
    ),
  );

  const topAds = computed(() =>
    adsStore.getActiveAds.filter(
      (ad) => ad.display === "top" || ad.display === "topfull",
    ),
  );
  // // 设计个定时器
  // setInterval(()=>{
  ...
  const transitionTo = (newState: ScreenState) => {
    try {
      console.log(
        `[Flow] 状态转换: ${currentScreenState.value} -> ${newState}`,
      );
  ...
  const startScreenSequence = () => {
    console.log("[Flow] 系统已禁用自动屏幕轮播功能");
    // 禁用自动屏幕轮播
    return;
  ...
  // 新增：将循环逻辑提取为独立函数
  const startNewCycle = () => {
    console.log("[Flow] 系统已禁用自动屏幕循环功能");
    // 禁用自动屏幕循环
    return;
  ...
  const handleRotationError = async (error: any) => {
    ...
    if (rotationRetryCount.value < maxRetryAttempts) {
      ...
      console.log(
        `[Flow] 尝试恢复轮播 (${rotationRetryCount.value}/${maxRetryAttempts})`,
      );
      await recoverFromError();
    } else {
