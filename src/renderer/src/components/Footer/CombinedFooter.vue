<template>
  <div class="w-full h-full bg-white">
    <div class="w-full h-full flex justify-between gap-3 p-3">
      <!-- Left Section - Today's Weather (调整尺寸更紧凑) -->
      <div
        class="w-1/4 flex flex-col bg-white rounded-xl border border-grey p-3 shadow-[0_2px_8px_rgba(0,0,0,0.06)]"
      >
        <div class="grid grid-cols-2 grid-rows-2 gap-2 h-full">
          <div class="flex flex-col justify-center items-center">
            <p class="text-xl font-semibold text-primary">
              {{
                new Date(
                  weatherData_today?.temperature?.recordTime || new Date(),
            ...
            <p class="text-xs font-medium text-neutral/80">
              {{
                new Date(
                  weatherData_today?.temperature?.recordTime || new Date(),
                ).toLocaleDateString("zh-CN", { weekday: "long" })
              }}
            </p>
          </div>
          ...
          <div class="flex flex-col justify-center items-center">
            ...
            <p class="text-xs font-medium text-neutral/80">
              {{ buildingStore.getLocation || "未知地點" }}
            </p>
          </div>
          ...
          <div class="flex justify-center items-center">
            <img
              ...
              alt="Weather Icon"
              class="w-12 h-12 object-contain filter-primary"
            />
          </div>

          <div class="flex flex-col justify-center gap-1 overflow-auto">
            <div
              v-if="Object.keys(weatherData_warning || {}).length > 0"
              v-for="(warning, key) in weatherData_warning"
              :key="key"
              class="px-1.5 py-0.5 bg-accent/5 rounded-lg border border-accent/10"
            >
              <p
                class="text-[9px] font-medium text-accent/90 text-center truncate"
              >
                {{ warning?.name?.trim() || "無" }}
              </p>
            </div>
            <div v-else class="px-2 py-1">
              <p class="text-lg font-medium text-center text-primary truncate">
                無
              </p>
            </div>
          </div>
      ...
      <!-- Right Section - Rotating Content (调整尺寸更紧凑) -->
      <Transition name="fade" mode="out-in">
        <div
          v-if="showWeather"
          class="flex-1 bg-white rounded-xl p-3 border border-grey shadow-[0_2px_8px_rgba(0,0,0,0.06)]"
        >
          <div class="grid grid-cols-5 gap-2 h-full">
            <div
              v-for="(
                forecast, index
              ) in weatherData_forecast?.weatherForecast?.slice(0, 5)"
              :key="index"
              class="flex flex-col items-center justify-between py-1"
            >
              <div
                :class="[
                  'px-2 py-0.5 rounded-lg text-xs font-medium w-full text-center',
                  index === 0
                    ? 'bg-primary/10 text-primary/90'
                    : 'bg-neutral/5 text-neutral/80',
                ]"
              >
                {{ index === 0 ? "Tmr" : forecast?.week?.slice(0, 3) }}
              </div>
              ...
              <div class="flex-1 flex items-center">
                <img
                  ...
                  alt="Forecast Icon"
                  class="w-10 h-10 object-contain filter-primary"
                />
              </div>
        ...
        <div v-else class="flex-1 flex gap-3">
          <div
            class="w-1/2 bg-white rounded-xl border border-grey p-3 shadow-[0_2px_8px_rgba(0,0,0,0.06)]"
          >
            <div class="h-full flex items-center gap-3">
              <div class="flex-shrink-0">
                <div
                  class="w-20 h-20 bg-neutral/5 rounded-lg flex items-center justify-center"
                >
                  <img
                    :src="`https://api.qrserver.com/v1/create-qr-code/?size=72x72&data=https://ismart.legend-in.com.hk/blg_cs_public/${buildingStore.getIsmartId}`"
                    alt="意見投訴二維碼"
              ...
              <div class="flex-1">
                <p class="text-xs font-medium text-primary mb-0.5">意見投訴</p>
                <p class="text-[10px] text-neutral/70">掃QRCode提交意見投訴</p>
              </div>
            </div>
          </div>
          ...
          >
            <div class="h-full flex items-center gap-3">
              <div class="flex-shrink-0">
                ...
                >
                  <img
                    :src="`https://api.qrserver.com/v1/create-qr-code/?size=72x72&data=https://ismart.legend-in.com.hk/regform/${buildingStore.getIsmartId}`"
                    alt="住戶登記二維碼"
                    class="w-16 h-16 object-contain"
                  />
                </div>
              ...
              <div class="flex-1">
                <p class="text-xs font-medium text-primary mb-0.5">住戶登記</p>
                <p class="text-[10px] text-neutral/70">掃QRCode進行住戶登記</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="isLoading" class="absolute top-2 right-2">
      <div
        class="inline-block h-3 w-3 animate-spin rounded-full border-2 border-primary border-r-transparent"
      ></div>
    </div>
  </div>
</template>
